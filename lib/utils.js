 ...
 */
export function compact(layout: Layout, l: LayoutItem, verticalCompact: boolean, horizontalCompact: boolean, horizontalFill: boolean, h:number): Layout {
  ...
  for (let i = 0, len = sorted.length; i < len; i++) {
    ...
    if (!l.static) {
      const oldItem = Object.assign({}, l)
      l = compactItem(compareWith, l, verticalCompact, horizontalCompact);
      if (horizontalFill) {
        const posIncrease = l.x - oldItem.x
        if (l.w !== l.minW && l.w !== l.maxW && l.w !== 1) {
          l.w -= posIncrease
        } else if (posIncrease < 0 && l.w !== l.maxW) {
          l.w -= posIncrease
        }
      }
